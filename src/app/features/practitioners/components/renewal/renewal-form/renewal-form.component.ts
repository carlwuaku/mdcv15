...
export class RenewalFormComponent {
  ...
  selectedPractitioner:PractitionerObject | null = null;
  ...
  getActions = (practitioner: PractitionerObject): DataActionsButton[]=> {
    ...
    if (practitioner.in_good_standing === "Not In Good Standing") {
      actions.push(
        { label: "Renew", type: "button", onClick: (practitioner: PractitionerObject) => this.selectForRenewal(practitioner) }
      )
    }
    else {
      actions.push(
        { label: "Edit", type: "link", link: `practitioners/renewal-form/`, linkProp: 'last_renewal_uuid' },
        { label: "Undo Renewal", type: "button", onClick: (practitioner: PractitionerObject) => this.undoRenewal(practitioner) },
      )
    }
    return actions;
  }
  ...
  getPractitioner(){
    this.dbService.get<PractitionerObject>(this.practitionerSearchBaseUrl + "/" + this.practitionerUuid)
    .pipe(take(1))
    .subscribe(data => {
  ...
  undoRenewal(object: PractitionerObject){
